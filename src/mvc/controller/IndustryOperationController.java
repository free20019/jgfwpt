...
public class IndustryOperationController {
    ...
    public String getVehiclesOperationNotOnline(HttpServletRequest request){
        ...
        String currentPage = request.getParameter("currentPage");
        String pageSize = request.getParameter("pageSize");
        return industryOperationService.getVehiclesOperationNotOnline(vehicle,area,sTime,eTime,type,currentPage,pageSize);
    }
    ...
    public String getVehiclesOperationNotOnlineExcel(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String a[] = {"车牌号码","区域","最后定位上传地点","最后定位上传时间","抓拍时间"};//导出列明
        String b[] = {"VEHICLE_NO","AREA_NAME","ADDRESS","LATEST_TIME","CATCH_TIME"};//导出map中的key
        String gzb = "运行中不在线车辆";//导出sheet名和导出的文件名
        String msg = industryOperationService.getVehiclesOperationNotOnline(vehicle,area,sTime,eTime,type,currentPage,pageSize);
        List<Map<String, Object>> list = downloadAct.str2list(msg);
        downloadAct.download(request,response,a,b,gzb,list);
        return null;
    }
    ...
    // 运行中不在线车辆统计
    @RequestMapping("/getVehiclesOperationNotOnlineStatistics")
    @ResponseBody
    public String getVehiclesOperationNotOnlineStatistics(HttpServletRequest request){
        String vehicle = request.getParameter("vehicle");
        String area = request.getParameter("area");
        String sTime = request.getParameter("sTime");
        String eTime = request.getParameter("eTime");
        String type = request.getParameter("type");
        return industryOperationService.getVehiclesOperationNotOnlineStatistics(vehicle,area,sTime,eTime,type);
    }

    // 导出运行中不在线车辆统计Excel
    @RequestMapping("/getVehiclesOperationNotOnlineStatisticsExcel")
    @ResponseBody
    public String getVehiclesOperationNotOnlineStatisticsExcel(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String vehicle = request.getParameter("vehicle");
        String area = request.getParameter("area");
        String sTime = request.getParameter("sTime");
        String eTime = request.getParameter("eTime");
        String type = request.getParameter("type");
        String a[] = {"车牌号码","区域","次数"};//导出列明
        String b[] = {"VEHICLE_NO","AREA_NAME","COUNT"};//导出map中的key
        String gzb = "运行中不在线车辆统计";//导出sheet名和导出的文件名
        String msg = industryOperationService.getVehiclesOperationNotOnlineStatistics(vehicle,area,sTime,eTime,type);
        List<Map<String, Object>> list = downloadAct.strlist(msg);
        downloadAct.download(request,response,a,b,gzb,list);
        return null;
    }
    ...
    // 司机签到签退查询
    @RequestMapping("/getDriverSignAndSignOffQuery")
    @ResponseBody
    public String getDriverSignAndSignOffQuery(HttpServletRequest request,String vehicle, String area, String icVehicle, String certificate, String sTime, String eTime){
        return industryOperationService.getDriverSignAndSignOffQuery(vehicle,area,icVehicle,certificate,sTime,eTime);
    }

    // 司机签到签退查询Excel
    @RequestMapping("/getDriverSignAndSignOffQueryExcel")
    @ResponseBody
    public String getDriverSignAndSignOffQueryExcel(HttpServletRequest request, HttpServletResponse response, String vehicle, String area, String icVehicle, String certificate, String sTime, String eTime) throws IOException {
        String a[] = {"当前车牌号","IC卡车牌号码","区域","服务资格证号","签到时间","签退时间"};//导出列明
        String b[] = {"OPERATION_VEHICLENO","IC_REGISTER_VEHICLENO","AREA_NAME","CERTIFICATE","SIGN_TIME","SIGN_OFF_TIME"};//导出map中的key
        String gzb = "司机签到签退查询";//导出sheet名和导出的文件名
        String msg = industryOperationService.getDriverSignAndSignOffQuery(vehicle,area,icVehicle,certificate,sTime,eTime);
        List<Map<String, Object>> list = downloadAct.strlist(msg);
        downloadAct.download(request,response,a,b,gzb,list);
        return null;
    }
    ...
    //运力需求热点
    @RequestMapping("/getcapacitydemandhotspot")
    @ResponseBody
    public String getcapacitydemandhotspot(HttpServletRequest request){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String stime = sdf.format(new Date(new Date().getTime()-1000*60*60*6));
        String etime = sdf.format(new Date());
        return industryOperationService.getcapacitydemandhotspot(stime, etime);
    }

    //运力需求热点(司机分布)
    @RequestMapping("/getcapacitydemandhotspotdriver")
    @ResponseBody
    public String getcapacitydemandhotspotdriver(HttpServletRequest request){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String stime = sdf.format(new Date())+" 00:00:00";
        String etime = sdf.format(new Date())+" 23:59:59";
        return industryOperationService.getcapacitydemandhotspotdriver(stime, etime);
    }
}
