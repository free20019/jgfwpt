...
public class PowerSchedulingServiceImpl implements PowerSchedulingService {
	...
	public String query(String username, String disptype, String stime, String etime) {
		...
		String sql1="select to_char(t.disp_time,'yyyy-mm-dd hh24:mi:ss') sendtime,to_char(t.vehi_count) nums,t.content,o.full_name username,'场站扬招' disptype,'' cars " +
		...
		String sql2="select to_char(db_time,'yyyy-mm-dd hh24:mi:ss') sendtime,numbers nums,msg content,user_account username,'手持监控' disptype, target cars " +
		...
		if("1".equals(disptype)){
			result=jdbcTemplate2.queryForList(sql1+"order by t.disp_time");
		}else if("2".equals(disptype)){
			result=jdbcTemplate2.queryForList(sql2+"order by db_time");
