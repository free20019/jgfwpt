...
public class ServicePlatformUsageServiceImpl implements ServicePlatformUsageService {
    ...
    public String GetEnterpriseSubCenterQuery(String realName, String userName, String stime, String etime) {
        String sql = "select u.REAL_NAME,u.USER_NAME,to_char(l.LOGIN_TIME,'yyyy-mm-dd hh24:mi:ss') time from ELECTRIC_TRICK_CAR.TB_USER@taxilink105 u, ELECTRIC_TRICK_CAR.TB_LOGIN_HISTORY@taxilink105 l where l.USER_ID = u.ID and LOGING_WAY = 1 and ACCOUNT_SORT = 0";
        if(userName != null&&!userName.isEmpty()&&!userName.equals("null")&&userName.length()>0){
            sql += " and u.USER_NAME = '"+userName+"'";
        }
        if(realName != null&&!realName.isEmpty()&&!realName.equals("null")&&realName.length()>0){
    ...
    public String getEnterpriseSubCenterStatistics(String realName, String userName, String stime, String etime) {
        String sql = "select u.REAL_NAME,u.USER_NAME,t.count from ELECTRIC_TRICK_CAR.TB_USER@taxilink105 u,(select USER_ID,count(USER_ID) count from ELECTRIC_TRICK_CAR.TB_LOGIN_HISTORY@taxilink105 where LOGING_WAY = 1 and ACCOUNT_SORT = 0";
    ...
    public String getHandheldMonitoringQuery(String userName, String stime, String etime) {
        String sql = "select USER_ACCOUNT,to_char(DB_TIME,'yyyy-mm-dd hh24:mi:ss') DB_TIME from HZ_TAXI.TB_LOGINMSG@taxilink105 where TYPE = 0 and LOGIN_RESULT = 0";
    ...
    public String GetHandheldMonitoringStatistics(String userName, String stime, String etime) {
        String sql = "select USER_ACCOUNT,count(*) count from HZ_TAXI.TB_LOGINMSG@taxilink105 where 1 = 1";
        if(userName!=null&&!userName.isEmpty()&&userName.length()>0&&!userName.equals("")){
            sql += " and USER_ACCOUNT = '"+userName+"'";
        }
        if(stime!=null&&!stime.isEmpty()&&stime.length()>0&&!stime.equals("")){
    ...
    public String getInstallationStatistics(String companyName) {
        ...
        String sql1 = "select t1.COMP_NAME,t1.count1 count1,case when t2.count2 > t1.count1 then t1.count1 else t2.count2 end count2,case when to_char(t1.count1-t2.count2) < '0' then 0 else t1.count1-t2.count2 end count3,case when round(t2.count2/t1.count1*100,2) >100 then 100 || '%' else round(t2.count2/t1.count1*100,2) || '%' end rate from (select COMP_NAME,count(COMP_NAME) count1 from VW_VEHICLE GROUP BY COMP_NAME) t1,(select COMPANY_NAME,count(COMPANY_NAME) count2 from TB_GLOBAL_VEHICLE where BUSINESS_SCOPE_NAME = '客运：出租车客运。' AND STATUS_NAME = '营运' AND PLATE_NUMBER LIKE '浙A%'";
        ...
        sql += ") g,(select count(*) c2 from (select * from (select PLATE_NUMBER from TB_GLOBAL_VEHICLE where BUSINESS_SCOPE_NAME = '客运：出租车客运。' AND STATUS_NAME = '营运' AND PLATE_NUMBER LIKE '浙A%' group by PLATE_NUMBER) a,(select vehi_no from TB_VEHICLE group by vehi_no) b where a.PLATE_NUMBER=b.vehi_no))v";
